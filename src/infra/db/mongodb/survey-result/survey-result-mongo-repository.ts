import { QueryBuilder, MongoHelper, ObjectId } from '@/infra/db/mongodb/helpers'
  ...
  async loadBySurveyId (surveyId: string, accountId: string): Promise<SurveyResultModel> {
    ...
    const query = new QueryBuilder()
      ...
      .group({
        ...
        isCurrentAccountAnswerCount: { $sum: '$answers.isCurrentAccountAnswerCount' }
      })
      .project({
        _id: 0,
        surveyId: '$_id.surveyId',
        ...
        answer: {
          ...
          isCurrentAccountAnswer: { $eq: ['$isCurrentAccountAnswerCount', 1] }
        }
      })
      .sort({ 'answer.count': -1 })
      .group({
      ...
      .build()
    const result = await surveyResultCollection.aggregate<SurveyResultModel>(query).toArray()
